...
if ($arrive->is_hotel_user()) {  $pagetitle ='فندق ' . $arrive->load_hotel() .'-'.' اجمالي  الليالي حسب تصنيف الفنادق ';}
if ($arrive->is_dta_user()) {  $pagetitle ='ولاية ' . $arrive->load_wilaya() .'-'.' اجمالي  الليالي حسب تصنيف الفنادق ';}
?>
<?php
    ...
    $(document).ready(function() {
        ...
        function load_data(from_date = '', to_date = '', filter_cat_etab = '') {
            var dataTable = $('#arrive_table').DataTable({
                ...
                "order": [],
                "ajax": {
                    url: "raport_nuitee_total_action.php",
                    type: "POST",
                    data: {
                        action: 'fetch',
                        from_date: from_date,
