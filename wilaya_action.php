...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('wilaya.nom_wilaya');
		$output = array();
		$main_query = "
		SELECT * FROM wilaya	";
		...
		if(isset($_POST["search"]["value"]))
		{
			...
			$search_query .= 'WHERE wilaya.nom_wilaya LIKE "%'.$_POST["search"]["value"].'%" ';
		...
		{
			$order_query = 'ORDER BY wilaya.wilaya_id ASC ';
		}
		$limit_query = '';
		...
		{
			$sub_array = array();
			$sub_array[] = $row["wilaya_id"];
			$sub_array[] = html_entity_decode($row["nom_wilaya"]);
			$sub_array[] = $row["tel_fax_wilaya"];
			$sub_array[] = $row["email_wilaya"];
			$status_button = '';
			if($row["status_wilaya"] == 'Active')
			{
				$status_button = '<button type="button" name="delete_button" class="btn btn-primary btn-sm delete_button" data-id="'.$row["wilaya_id"].'" data-status="'.$row["status_wilaya"].'">'.$row["status_wilaya"].'</button>';
			}
			else
			{
				$status_button = '<button type="button" name="delete_button" class="btn btn-danger btn-sm delete_button" data-id="'.$row["wilaya_id"].'" data-status="'.$row["wilaya_id"].'">'.$row["wilaya_id"].'</button>';
			}
			$sub_array[] = $status_button;
			$sub_array[] = '
			<div align="center">
			<button type="button" name="edit_button" class="btn btn-warning btn-sm edit_button" data-id="'.$row["wilaya_id"].'"><i class="fas fa-edit"></i></button>
			</div>';
			$data[] = $sub_array;
		}
		$output = array(
