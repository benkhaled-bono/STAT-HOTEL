...
if (isset($_POST["action"])) {
    if ($_POST["action"] == 'fetch') {
        ...
        } elseif ($arrive->is_hotel_agent_user()) {
            $main_query .= "
    ...
    if ($_POST["action"] == 'fetch_view') {
        ...
        foreach ($result as $row) {
            ...
            $data['continentv']             = $row['continent_ID'];
    ...
    ");
        ...
        if ($dif_days > $dif_days_of_completed_sejour) {
            ...
            $nuite_mois     = ((int)$sejour_mois * (int)$nombre);
            $nuite_reste     = $sejour_reste * $nombre;
        }
        $data = array(
    ':date_rap'                  => $date_rap,
...
} elseif (isset($_POST['nationId']) && !empty($_POST['nationId'])) {

    // Fetch city name base on state id
    $query = "SELECT * FROM continent WHERE nation_ID = " . $_POST['nationId'];
    $result = $con->query($query);
    if ($result->num_rows > 0) {
        echo  '<option value="">اختر القارة</option>';
        while ($row = $result->fetch_assoc()) {
            echo '<option value="' . $row['continent_id'] . '">' . $row['nom_continent'] . '</option>';
        }
    } else {
        echo '<option value="">القارة غير موجودة</option>';
    }
}
...
} elseif (isset($_POST['nationID']) && !empty($_POST['nationID'])) {    // Fetch city name base on state id
    $query = "SELECT * FROM continent WHERE nation_ID = " . $_POST['nationID'];
    $result = $con->query($query);
    if ($result->num_rows > 0) {
        echo  '<option value="">اختر القارة</option>';
