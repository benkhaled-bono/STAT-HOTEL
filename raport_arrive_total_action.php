...
if (isset($_POST["action"])) {
    if ($_POST["action"] == 'fetch') {
        ...
        $output = array();
        $main_query = "	SELECT SUM((stat_arrive.nombre_arrive)) as 'المجموع العام', 
        SUM((CASE WHEN section_arrive.section_arrive_id = 1 THEN stat_arrive.nombre_arrive ELSE 0 END)) AS 'الاجانب المقيمون', 
        SUM((CASE WHEN section_arrive.section_arrive_id = 2 THEN stat_arrive.nombre_arrive ELSE 0 END)) AS 'الجزائريون المقيمون', 
        SUM((CASE WHEN section_arrive.section_arrive_id = 3 THEN stat_arrive.nombre_arrive ELSE 0 END)) AS 'الاجانب غير المقيمون', 
        SUM((CASE WHEN section_arrive.section_arrive_id = 4 THEN stat_arrive.nombre_arrive ELSE 0 END)) AS 'الجزائريون غير المقيمون'
        FROM stat_arrive
